...
function SpeakersSection({
    ...
    const { onDisjoinEventSpeaker } = useActions();
    const {
        handleChange,
        setFieldValue,
        handleSubmit,
    ...
    useEffect(() => {
        ...
        speakers.forEach((_speaker, index) => {
            const {
                topic,
                endTime,
                startTime,
        ...
        addMultipleSpeakers(multipleSpeakers);
    }, []);
    ...
    const onLayout = (event) => {
        ...
        if (speakersInfo.onMount) {
            setSpeakersInfo((prev) => ({ ...prev, onLayout: true }));
        }
    };

    const removeSpeaker = async (idx) => {
        try {
            let newSpeakers = formValues;
            const speakerToDelete = newSpeakers[idx];
            if (speakerToDelete.isNew === false) {
                const payload = {
                    eventId,
                    eventSpeakerJoinId: speakerToDelete.eventSpeakerJoinId,
                };
                await onDisjoinEventSpeaker(payload);
            }
            Alert.alert('Speaker removed from the event');
